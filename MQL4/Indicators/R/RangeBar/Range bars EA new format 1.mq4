...
double open(int pShift)   { return (iOpen(Symbol()  , PERIOD_M1, pShift)); }
double close(int pShift)  { return (iClose(Symbol() , PERIOD_M1, pShift)); }
double high(int pShift)   { return (iHigh(Symbol()  , PERIOD_M1, pShift)); }
double low(int pShift)    { return (iLow(Symbol()   , PERIOD_M1, pShift)); }
...
int start() {
   ...
   while (IsStopped() == FALSE) {
      ...
      if (RefreshRates()) {
         ...
         startLow = MathMin(startLow, Close[0]);
         startHigh = MathMax(startHigh, Close[0]);
         startVolume++;
         if (MathRound((startHigh - startLow) / Point / minSize) <= MathRound(BarsRange / minSize)) {
            tmpVolume = volume(0);
            startClose = close(0);
         } else {
            if (CompareDoubles(startLow, Close[0])) {
               if (ShowGaps) startLow = NormalizeDouble(startHigh - BarsRange * Point, Digits);
               else startLow = NormalizeDouble(startLow + minSize * Point, Digits);
               startClose = startLow;
            }
            if (CompareDoubles(startHigh, Close[0])) {
               if (ShowGaps) startHigh = NormalizeDouble(startLow + BarsRange * Point, Digits);
               else startHigh = NormalizeDouble(startHigh - minSize * Point, Digits);
               startClose = startHigh;
            }
