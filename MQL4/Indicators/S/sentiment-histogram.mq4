...
double            g_trendupplus[];
double            g_trenddnplus[];
double            g_trendupminus[];
double            g_trenddnminus[];
double            g_sentBull[];
double            g_sentBear[];
double            g_tempBull[];
double            g_tempBear[];
...
bool BuffersBind()
{
   ...
   if (!SetIndexBuffer(0, g_trendupplus)      ||
       !SetIndexBuffer(1, g_trendupminus)     ||
       !SetIndexBuffer(2, g_trenddnplus)      ||
       !SetIndexBuffer(3, g_trenddnminus)     ||
       !SetIndexBuffer(4, g_sentBull)         ||
       !SetIndexBuffer(5, g_sentBear)         ||
       !SetIndexBuffer(6, g_tempBull)         ||
       !SetIndexBuffer(7, g_tempBear))
   {
...
{
   ArrayInitialize(g_trenddnplus, EMPTY_VALUE);
   ArrayInitialize(g_trendupplus, EMPTY_VALUE);
   ArrayInitialize(g_trenddnminus, EMPTY_VALUE);
   ArrayInitialize(g_trendupminus, EMPTY_VALUE);
...
void ShowIndicatorData(int limit)
{
   for (int i = limit; i >= 0; i--)
   {
      ...
      double diff = g_sentBull[i] - g_sentBear[i];
      
      if (diff > 0.0)
      {
         if (g_sentBull[i] >= g_sentBull[i + 1]) 
            g_trendupplus[i] = diff;
         else 
            g_trendupminus[i] = diff; 
         continue;
      }
      
      if (g_sentBear[i] >= g_sentBear[i + 1])
         g_trenddnplus[i] = diff;
      else 
         g_trenddnminus[i] = diff;
   }
}
//+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
