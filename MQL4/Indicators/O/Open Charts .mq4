...
extern string templatename="ExactTrading//default"; //Choose template name
...
void deinit() {
   ...
   ArrayFree(PAIRS); ArrayResize(PAIRS, 0);
   ArrayFree(INDEX); ArrayResize(INDEX,0);

   Close_gui();
}

void InitPairs( PAIR &myPAIRS[] ) {

   int iPair = 0;
   string tempSymbol;
   ArrayResize( myPAIRS, 0 );
   ...
   for ( int i = 0; i < recordCount; i++ ) {
      ...
      if ( MarketInfo ( tempSymbol, MODE_BID ) > 0 ) {
         ArrayResize( myPAIRS, iPair+1 );
         myPAIRS[iPair].name = tempSymbol;
         myPAIRS[iPair].xx   = 20  + (int)MathMod((double)iPair,4.0)*105;
         myPAIRS[iPair].yy   = 110 + (int)(iPair / 4)*25;
         //PrintFormat("Symbol: %s, xx = %d, yy = %d", myPAIRS[iPair].name, myPAIRS[iPair].xx, myPAIRS[iPair].yy);
         iPair++;
      }
       ...
       if(sparam=="gui_ok" && ObjectGet(sparam,OBJPROP_STATE))
           {//create new charts in order
            for(int p=0; p<pairCount; p++)
                {if(INDEX[p]>=0){cn=ChartOpen(PAIRS[p].name,period); ChartApplyTemplate(cn, FileName); /*ChartIndicatorDelete(cn,0,"Open Charts In Alphabetical Order Indicator");*/ Close_gui();} }  
...
void Open_gui()
   {string name="gui_";
    ...
    RectLabelCreate(0,name+"R", 0, x, y,445,110+((int)MathCeil(pairCount/4.0))*25, clrBlue, BORDER_FLAT, CORNER_LEFT_UPPER, clrWhite, STYLE_SOLID, 4, false, false, true, 0) ;
    ...
    for(int c=0; c<pairCount; c++)
        {ButtonCreate(0,name+IntegerToString(c),0, PAIRS[c].xx, PAIRS[c].yy, 100, 20, CORNER_LEFT_UPPER, PAIRS[c].name, "Arial Black", 11, clrtx, clrbg, clrbg, false, false, false, false, 0);}
   }
//------------------------------------------------------------------+
void Close_gui() 
