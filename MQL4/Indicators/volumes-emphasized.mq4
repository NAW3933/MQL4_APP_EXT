...
#property indicator_color3 C'40,40,40'   // Normal volume bars
#property indicator_color4 DarkSlateBlue            // Volumes between 0% avg. & first graduation #
#property indicator_color5 SlateBlue       // Volumes between first & second graduation #
#property indicator_color6 CornflowerBlue  // Volumes between second graduation # and 100% of avg.
#property indicator_color7 LimeGreen    // Volumes greater than 100% of average
...
extern double Enter_ShrinkNumber              = 3;
...
extern color  BelowAverageTextColor_0         = C'40,40,40';
extern color  AboveAverageTextColor_1         = DarkSlateBlue;
extern color  AboveAverageTextColor_2         = SlateBlue;
extern color  AboveAverageTextColor_3         = CornflowerBlue;
extern color  AboveAverageTextColor_4         = LimeGreen;
extern int    AveragePeriod                   = 14;
extern int    AverageMethod                   = MODE_SMA;
extern int    Text_in_window_0123             = 0;
extern int    Text_in_corner_0123             = 0;
extern int    __Text_Horizontal_Indent        = 5;
extern int    __Text_Vertical_Indent          = 170;
...
int init()
{
   ...
   SetIndexBuffer(7, VolAverage);   //This draws the bars over 100% of avg.
...
int start()
{
   ...
   for(i = limit; i >= 0; i--)
   {
      ...
      if (nvo<0){        
         if(Display_volume_percentages_text) Label("Volume is "+Nvo+" % AVG",BelowAverageTextColor_0);}
         
      else{if (nvo<Enter_Number_1){ 
         if(Display_volume_percentages_text){Label("Volume is "+Nvo+" % AVG",AboveAverageTextColor_1);}
         VolBufferH4[i]=Volume[i];}
          
      else{if (nvo<Enter_Number_2){ 
         if(Display_volume_percentages_text){Label("Volume is "+Nvo+" % AVG",AboveAverageTextColor_2);}
         VolBufferH5[i]=Volume[i]; }
         
      else{if (nvo<100){
         if(Display_volume_percentages_text){Label("Volume is "+Nvo+"% AVG",AboveAverageTextColor_3);}
         VolBufferH6[i]=Volume[i]; }
         
      else if (nvo>=100){
         if(Display_volume_percentages_text){Label("Volume is "+Nvo+" % AVG",AboveAverageTextColor_4);}
   ...
   if (AveragePeriod>0) for(i = limit; i >= 0; i--) VolAverage[i] = iMAOnArray(VolBufferH3,0,AveragePeriod,0,AverageMethod,i);
   return(0);
   }
...
void Label(string text, color Color)  
   {
      ...
      ObjectSetText("voltxt", text, text_size, "Arial Black");    
      }
   }
